...
def is_secret(thing: str) -> bool:
    ...
    if thing == "MUSTCENSORE":
        return True
    try:
        secrets = [x.lower().rstrip() for x in environ["secrets"].split(":")]
...
def main(argv=None):
    things = {
        "architecture":
        censore_if_needed("architecture", platform.architecture()),
        "machine":
        censore_if_needed("machine", platform.machine()),
        "node":
        censore_if_needed("node", platform.node()),
        "platform":
        censore_if_needed("platform", platform.platform()),
        "processor":
        censore_if_needed("processor", platform.processor()),
        "python_build":
        censore_if_needed("python_build", platform.python_build()),
        "python_compiler":
        censore_if_needed("python_compiler", platform.python_compiler()),
        "python_branch":
        censore_if_needed("python_branch", platform.python_branch()),
        "python_implementation":
        censore_if_needed("python_implementation",
                          platform.python_implementation()),
        "python_revision":
        censore_if_needed("python_revision", platform.python_revision()),
        "python_version":
        censore_if_needed("python_version", platform.python_version()),
        "system_alias":
        censore_if_needed(
            "system_alias",
    ...
    if platform.system().lower().rstrip() == "windows":
        things["win32_ver"] = censore_if_needed("win32_ver",
                                                platform.win32_ver())
        things["win32_edition"] = censore_if_needed("win32_edition",
                                                    platform.win32_edition())
        things["win32_is_iot"] = censore_if_needed("win32_is_iot",
                                                   platform.win32_is_iot())

    if platform.system().lower().rstrip().find("mac") != -1:
        things["mac_ver"] = censore_if_needed("mac_ver", platform.mac_ver())

    if platform.system().lower().rstrip().find("unix") != -1:
        things["libc_ver"] = censore_if_needed("libc_ver", platform.libc_ver())

    print(f"OS                   : {name}")

    if things["architecture"]:
        print(
            f"Architecture         : {things['architecture'][1]} {things['architecture'][0]}"
        )

    if things["system_alias"]:
        print(f"System alias         : {' '.join(things['system_alias'])}")

    if things["win32_ver"]:
        print(f"Windows version      : {' '.join(things['win32_ver'])}")
    ...
    print(
        f"Libc version         : {' '.join(things['libc_ver']) if things['libc_ver'] != () else 'N/A'}"
    )
    ...
    if things["python_build"]:
        print(
            f"Python               : {things['python_build'][0]} ({things['python_build'][1]})"
        )
    ...
    if things["python_implementation"]:
        print(f"Python implementation: {things['python_implementation']}")
